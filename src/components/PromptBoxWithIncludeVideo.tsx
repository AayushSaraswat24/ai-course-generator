...
import { ChevronUp, ArrowUp, Video } from "lucide-react";

interface PromptBoxProps {
  onSubmit: (prompt: string, level: string, includeVideo: boolean) => void;
}
...
export default function PromptBoxWithIncludeVideo({ onSubmit }: PromptBoxProps) {
  ...
  const handleSend = () => {
    ...
    if (!prompt) return;
    onSubmit(prompt, selectedLevel, includeVideo);
    if (textarea) {
      textarea.value = "";
      textarea.style.height = "auto";
    }
    ...
    <div className="fixed bottom-6 p-2 left-0 w-full flex justify-center z-50">
      <div className="relative w-full max-w-3xl bg-gray-100 dark:bg-neutral-800 rounded-2xl shadow-lg border border-gray-300 dark:border-gray-700 p-4 pt-10">
        ...
        <div className="absolute -top-3 left-4 flex items-center gap-2">
          ...
          <button
            onClick={() => setIncludeVideo((prev) => !prev)}
        ...
        <div className="flex items-end">
          <textarea
            ref={textareaRef}
            onInput={handleInput}
            // ensuring enter + shift not press together if yes then it is to add new line not to submit .
            onKeyDown={(e) => {
