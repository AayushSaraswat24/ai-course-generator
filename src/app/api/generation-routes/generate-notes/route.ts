...
import { courseGenerator } from "@/lib/helper/notesGenerator";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const subTopics = JSON.stringify([topics.mainTopic, ...topics.subtopics.map((item) => item.title)]);

   
    const notesStreamPromise = courseGenerator(topics.mainTopic, subTopics, userKnowledge);

    const [ytVideos, notesStream] = await Promise.all([ytVideosPromise, notesStreamPromise]);
